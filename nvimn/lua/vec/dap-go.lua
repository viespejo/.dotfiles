...
local function setup_go_adapter(dap)
  dap.adapters.go = function(callback, config)
    ...
    local port = "38697"
    if config.mode == 'remote' then --we do not need to start a 'dlv dap' server
      port = tonumber(vim.fn.input('Port Remote Server: '))
    else
...
local function setup_go_configuration(dap)
  dap.configurations.go = {
		...
		{
			type = "go",
			name = "Attach remote dlv headless server",
			request = "attach",
			mode = "remote",
			host = "127.0.0.1",
			-- port = "38699",
		...
		{
			type = "go",
			name = "Debug current file (internal DAP server)",
			request = "launch",
			program = "${file}",
		},
		{
			type = "go",
			name = "Debug (folder) (internal DAP server)",
			request = "launch",
			program = "${workspaceFolder}",
		},
		{
			type = "go",
			name = "Test current file (internal DAP server)",
			request = "launch",
			mode = "test",
			program = "${file}",
		},
		{
			type = "go",
			name = "Test (folder) (internal DAP server)",
			request = "launch",
			mode = "test",
			program = "${workspaceFolder}",
		},
